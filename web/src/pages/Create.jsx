...
export default function Create({ step, setStep }) {
  ...
  // Signature / watermark
  const [signature, setSignature] = useState({ enabled: false, text: "@ViralCutAI", pos_x: 50, pos_y: 92, size: 34, color: "#FFFFFF", opacity: 75 });
  ...
  // Fetch the transcript once pre-transcription has produced one.
  useEffect(() => {
  ...
  // Keep curTime/vidDuration in sync with the video element.
  useEffect(() => {
  ...
  // Auto-generate on entering Screen 6 (Review) if not already done.
  useEffect(() => {
    if (step === 6 && !busy && !generatedRef.current && sourceReady) {
  ...
  async function generate() {
    ...
    try {
      ...
      closeRef.current = streamProgress(job_id, (sn) => {
        setSnap(sn);
        if (sn.clips) { setClips(sn.clips); setClipId(job_id); }
  ...
  function restart() {
    setStep(1);
    setUrl(""); setUpload(null); setUpPct(null);
    if (objUrl) { URL.revokeObjectURL(objUrl); setObjUrl(null); }
    setSource("url"); setClips([]); setSnap(null); setError("");
  ...
  const Breadcrumb = () => (
    <div className="wizard-breadcrumb">
      {WIZARD_LABELS.map((lbl, i) => (
        <span key={lbl} className={"crumb" + (step === i + 1 ? " active" : step > i + 1 ? " done" : "")}>
          {i > 0 && <span className="crumb-sep">›</span>}
          <span className="crumb-lbl">{lbl}</span>
        </span>
      ))}
    </div>
  );

  // Reframe overlay (renders on top of any screen)
  if (reframeTarget) {
    return (
      <ReframeEditor
        clipId={clipId}
        clip={reframeTarget.clip}
        targetAspect={targetAspectNum(aspect, fit)}
        initialKeyframes={reframeCache.current[`${clipId}:${reframeTarget.clip.index}`] || []}
        onClose={() => setReframeTarget(null)}
        onSaved={(idx, kf, result) => {
          reframeCache.current[`${clipId}:${idx}`] = kf;
          if (result?.clip) {
            setClips((cs) => cs.map((c, i) => (i === reframeTarget.idx ? result.clip : c)));
          }
        }}
      />
    );
  ...
  const commonProps = { studio, language, media, sourceReady, aspect, fit, barText, signature, setSig, videoRef };
    ...
    <>
      ...
      {step === 1 && (
        <Screen1Input
          ...
          doUpload={doUpload} onClear={clearSource} sourceReady={sourceReady} error={error}
      ...
      {step === 2 && (
        <Screen2Settings
          {...commonProps}
          prepView={prepView} outputAspect={outputAspect} setOutputAspect={setOutputAspect}
          ...
          language={language} changeLanguage={changeLanguage} device={device} setDevice={setDevice} devices={devices}
          numClips={numClips} setNumClips={setNumClips} clipLen={clipLen} setClipLen={setClipLen}
          onBack={() => setStep(1)} onNext={() => setStep(3)} nextEnabled={sourceReady}
        />
      )}
      ...
      {step === 3 && (
        <Screen3Captions
          {...commonProps}
          onFontUpload={onFontUpload} prepView={prepView}
          transcript={transcript} curTime={curTime} seekTo={seekTo}
      ...
      {step === 5 && (
        <Screen5Music
          {...commonProps}
          tracks={tracks} musicTrack={musicTrack} musicVolume={musicVolume}
          musicDuck={musicDuck} musicStart={musicStart} musicSuggest={musicSuggest}
          onTrack={(t) => { setMusicTrack(t); setMusicStart(0); }}
          onVolume={setMusicVolume} onDuck={setMusicDuck} onStart={setMusicStart}
          onMusicUpload={onMusicUpload} onRefresh={refreshMusic}
      ...
      {step === 6 && (
        <Screen6Review
          ...
          onOpenReframe={(clip, idx) => setReframeTarget({ clip, idx })}
          onBack={() => { setStep(5); generatedRef.current = false; }}
          onNext={() => setStep(7)}
        />
      )}
