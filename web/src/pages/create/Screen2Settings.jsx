    ...
    <div className="wizard-screen">
      <div className="w3-grid">
        <div className="card w3-left">
          ...
          {isSquare && (
            <>
              <label className="fieldlabel" style={{ marginTop: 14 }}>Corners</label>
              <div className="toggle">
              ...
              <label className="fieldlabel" style={{ marginTop: 14 }}>Title text (top) — Shift+Enter for a new line</label>
          ...
          <div style={{ marginTop: 14, display: "grid", gap: 14 }}>
            <div>
              <label className="fieldlabel">Number of clips</label>
            ...
            <div>
              <label className="fieldlabel">Compute device</label>
              <select value={device} onChange={(e) => setDevice(e.target.value)}>
                {["auto", ...devices.filter((d) => d !== "auto")].filter((v, i, a) => a.indexOf(v) === i).map((d) => (
                  <option key={d} value={d}>{d === "cuda" ? "GPU (CUDA)" : d.toUpperCase()}</option>
                ))}
              </select>
            </div>
          </div>
      ...
      <div className="wizard-nav">
        ...
        <button className="btn btn-primary" onClick={onNext} disabled={!nextEnabled}>Next →</button>
      </div>
    </div>
  );
}
